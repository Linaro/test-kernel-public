	...
	unsigned int vodspreg, unsigned int value)
{
	...
	rt5625_write(codec, RT5625_VODSP_REG_CMD, VODSP_WRITE_ENABLE |
		VODSP_CMD_MW);
	mdelay(10);
	return 0;
...
static int ConfigPcmVoicePath(struct snd_soc_codec *codec,
	unsigned int bEnableVoicePath, unsigned int mode)
{
	if (bEnableVoicePath) {
...
static int set_vodsp_aec_path(struct snd_soc_codec *codec, unsigned int mode)
{
	...
	case ANALOG_IN_ANALOG_OUT:
		rt5625_write_mask(codec, 0x26, 0x0300, 0x0300);
		rt5625_write_mask(codec, RT5625_VODSP_PDM_CTL,
				VODSP_RXDP_PWR | VODSP_RXDP_S_SEL_ADCL |
				VOICE_PCM_S_SEL_AEC_TXDP,
				VODSP_RXDP_PWR | VODSP_RXDP_S_SEL_MASK |
				VOICE_PCM_S_SEL_MASK);
		rt5625_write_mask(codec, RT5625_DAC_ADC_VODAC_FUN_SEL,
				ADCR_FUNC_SEL_PDM|VODAC_SOUR_SEL_VODSP_TXDC |
				DAC_FUNC_SEL_VODSP_TXDP|ADCL_FUNC_SEL_VODSP,
				ADCR_FUNC_SEL_MASK|VODAC_SOUR_SEL_MASK |
				DAC_FUNC_SEL_MASK|ADCL_FUNC_SEL_MASK);
		rt5625_write_mask(codec, RT5625_VODSP_CTL,
...
static int enable_vodsp_aec(struct snd_soc_codec *codec,
	unsigned int VodspAEC_En, unsigned int AEC_mode)
{
	int  ret = 0;
	...
	if (VodspAEC_En != 0) {
		...
		rt5625_write_mask(codec, RT5625_PWR_MANAG_ADD1,
			PWR_I2S_INTERFACE, PWR_I2S_INTERFACE);
...
static const struct snd_kcontrol_new rt5625_left_hp_mixer_controls[] = {
	...
	SOC_DAPM_SINGLE("HIFI DAC Playback Switch", RT5625_DAC_AND_MIC_CTRL,\
		3, 1, 1),
};
	...
	struct snd_kcontrol *k, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		rt5625_write_mask(codec, 0x02, 0x0000, 0x8080);
		/* power on spk amp */
		rt5625_write_mask(codec, 0x3a, 0x0400, 0x0400);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* power off spk amp */
		rt5625_write_mask(codec, 0x3a, 0x0000, 0x0400);
		rt5625_write_mask(codec, 0x02, 0x8080, 0x8080);
...
static const struct snd_soc_dapm_widget rt5625_dapm_widgets[] = {
	...
	SND_SOC_DAPM_PGA("Left LineIn PGA", RT5625_PWR_MANAG_ADD3,\
		7, 0, NULL, 0),
	SND_SOC_DAPM_PGA("Right LineIn PGA", RT5625_PWR_MANAG_ADD3,\
		6, 0, NULL, 0),
	...
	SND_SOC_DAPM_MUX("SPKOUT Mux", SND_SOC_NOPM,\
		0, 0, &rt5625_spkout_mux_out_controls),
	SND_SOC_DAPM_MUX("HPLOUT Mux", SND_SOC_NOPM,\
		0, 0, &rt5625_hplout_mux_out_controls),
	SND_SOC_DAPM_MUX("HPROUT Mux", SND_SOC_NOPM,\
		0, 0, &rt5625_hprout_mux_out_controls),
	SND_SOC_DAPM_MUX("AUXOUT Mux", SND_SOC_NOPM,\
		0, 0, &rt5625_auxout_mux_out_controls),

	SND_SOC_DAPM_PGA_E("SPKL Out PGA", VIRTUAL_REG_FOR_MISC_FUNC, 4, 0,\
		NULL, 0, spk_pga_event,\
		SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_PGA_E("SPKR Out PGA", VIRTUAL_REG_FOR_MISC_FUNC, 5, 0,\
		NULL, 0, spk_pga_event,\
		SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_PGA_E("HPL Out PGA", VIRTUAL_REG_FOR_MISC_FUNC, 6, 0,\
		NULL, 0, hp_pga_event,\
		SND_SOC_DAPM_POST_PMD | SND_SOC_DAPM_POST_PMU),
	SND_SOC_DAPM_PGA_E("HPR Out PGA", VIRTUAL_REG_FOR_MISC_FUNC, 7, 0,\
		NULL, 0, hp_pga_event,\
		SND_SOC_DAPM_POST_PMD | SND_SOC_DAPM_POST_PMU),
	SND_SOC_DAPM_PGA_E("AUX Out PGA", RT5625_PWR_MANAG_ADD3, 14, 0,\
		...
		for (i = 0; i < ARRAY_SIZE(codec_master_pll1_div); i++) {
			...
			(freq_out == codec_master_pll1_div[i].pll_out)) {
				...
				rt5625_write_mask(codec,\
					RT5625_PWR_MANAG_ADD2, 0x8000, 0x8000);
				rt5625_write_mask(codec,\
					RT5625_GEN_CTRL_REG1, 0x8000, 0x8000);
				ret = 0;
			}
		}
