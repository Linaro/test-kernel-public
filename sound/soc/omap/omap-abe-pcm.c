...
static int omap_abe_dl1_enabled(struct omap_abe *abe)
{
	...
	return omap_abe_port_is_enabled(abe->abe,
				abe->dai.port[OMAP_ABE_BE_PORT_PDM_DL1]) +
		omap_abe_port_is_enabled(abe->abe,
		...
		struct snd_soc_dai *dai, int stream)
{
	...
	case OMAP_ABE_DAI_BT_VX:
		if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			if (omap_abe_port_is_enabled(abe->abe,
					abe->dai.port[OMAP_ABE_BE_PORT_BT_VX_DL]))
				return;
...
static int omap_abe_dai_probe(struct snd_soc_dai *dai)
{
	...
	abe->abe = omap_abe_port_mgr_get();
	if (!abe->abe)
		goto err;

	for (i = 0; i <= OMAP_ABE_MAX_PORT_ID; i++) {

		abe->dai.port[i] = omap_abe_port_open(abe->abe, i);
...
err_port:
	for (--i; i >= 0; i--)
		omap_abe_port_close(abe->abe, abe->dai.port[i]);
	omap_abe_port_mgr_put(abe->abe);
err:
	return -ENODEV;
}
