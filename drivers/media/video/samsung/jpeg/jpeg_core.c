...
int jpeg_set_dec_param(struct jpeg_control *ctrl)
{
	...
	jpeg_set_stream_buf(&ctrl->mem.stream_data_addr, ctrl->mem.phy_base);
	jpeg_set_stream_addr(ctrl->reg_base, ctrl->mem.stream_data_addr);
	jpeg_set_frame_buf(&ctrl->mem.frame_data_addr, ctrl->mem.phy_base);
...
int jpeg_exe_enc(struct jpeg_control *ctrl)
{
	...
	jpeg_info("encode img in_fmt(%d) width(%d) height(%d)\n",
			ctrl->enc_param.in_fmt , ctrl->enc_param.width,
			ctrl->enc_param.height);
