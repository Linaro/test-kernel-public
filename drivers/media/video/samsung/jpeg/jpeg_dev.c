...
#if defined(CONFIG_S5P_SYSMMU_JPEG)
	s5p_vcm_turn_on(jpeg_ctrl->mem.vcm_info.sysmmu_vcm);
	jpeg_dbg("sysmmu on\n");
#if defined(CONFIG_VCM)
	vcm_activate(jpeg_ctrl->mem.vcm_info.sysmmu_vcm);
#endif /* CONFIG_VCM */
...
#if defined(CONFIG_S5P_SYSMMU_JPEG)
#if defined(CONFIG_VCM)
	vcm_deactivate(jpeg_ctrl->mem.vcm_info.sysmmu_vcm);
#endif /* CONFIG_VCM */
	s5p_vcm_turn_off(jpeg_ctrl->mem.vcm_info.sysmmu_vcm);
	jpeg_dbg("sysmmu off\n");
#endif
			...
			unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_SET_DEC_IN_BUF:
	case IOCTL_SET_ENC_OUT_BUF:
		jpeg_set_stream_base(arg);
		break;

	case IOCTL_SET_DEC_OUT_BUF:
	case IOCTL_SET_ENC_IN_BUF:
		jpeg_set_frame_base(arg);
		break;
...
{
#if defined(CONFIG_S5P_SYSMMU_JPEG)
#if !defined(CONFIG_S5P_VMEM) && !defined(CONFIG_VIDEO_UMP)
...
static int jpeg_probe(struct platform_device *pdev)
{
	...
	/* ioremap */
	jpeg_ctrl->reg_base = ioremap(res->start, resource_size(res));
	...
	/*
	jpeg_ctrl->pd = regulator_get(&pdev->dev, "samsung_pd");
	if (IS_ERR(jpeg_ctrl->pd)) {
		jpeg_err("failed to find jpeg regulator source\n");
		return -ENODEV;
		goto err_regulator;
	}
	regulator_enable(jpeg_ctrl->pd);
	*/
...
#if defined(CONFIG_S5P_SYSMMU_JPEG)
#if defined(CONFIG_VCM)
	jpeg_ctrl->mem.vcm_info.sysmmu_vcm = vcm_create_unified(SZ_64M,
							VCM_DEV_JPEG,
							NULL);
	/* sysmmu_set_tablebase_pgd(SYSMMU_JPEG,
				__pa(jpeg_ctrl->mem.vcm_info.sysmmu_pgd)); */
#else	/* jpeg hw uses kernel virtual address */
	...
	printk("\nJPEG driver successfully probed..\n");
	return 0;
