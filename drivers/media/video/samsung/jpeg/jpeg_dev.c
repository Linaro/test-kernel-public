...
int jpeg_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	page_frame_no = __phys_to_pfn(jpeg_ctrl->mem.phy_base);
...
static int jpeg_probe(struct platform_device *pdev)
{
	...
	ret = jpeg_mem_init(&pdev->dev, &jpeg_ctrl->mem);
	...
	dev_info(&pdev->dev, "JPEG driver probe successful\n");
	return 0;
...
{
	free_irq(jpeg_ctrl->irq_no, pdev);
	mutex_destroy(&jpeg_ctrl->lock);
	iounmap(jpeg_ctrl->reg_base);

	jpeg_mem_free(&pdev->dev, &jpeg_ctrl->mem);
