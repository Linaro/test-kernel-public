				...
				v4l2_std_id *norm)
{
	...
	tvout_dbg("standard id=0x%X, name=\"%s\"\n", (u32) std_id, \
					s5p_tvout_tvif_standard[i].name);
					  ...
					  struct v4l2_format *a)
{
	...
	s5p_vp_ctrl_set_dest_win(rect->left, rect->top, \
					rect->width, rect->height);
	return 0;
}

static int s5p_tvout_vo_g_crop(struct file *file, void *fh, \
						struct v4l2_crop *a)
{
	switch (a->type) {
