					...
					bool ipc)
{
	...
	s5p_vp_set_src_dest_size(src_win.w, src_win.h, dst_win.w, \
						dst_win.h, ipc);
}
				    ...
				    u32 img_w, enum s5p_vp_src_color color_t)
{
	...
	if (s5p_vp_ctrl_get_src_scan_mode() == INTERLACED) {
		...
		if (color_t == VP_SRC_COLOR_NV12IW || color_t == \
						VP_SRC_COLOR_NV21IW) {
...
static int s5p_vp_ctrl_set_reg(void)
{
	...
	s5p_vp_set_brightness_contrast(pp_param->brightness, \
							pp_param->contrast);
...
int s5p_vp_ctrl_constructor(struct platform_device *pdev)
{
	...
	s5p_vp_ctrl_private.clk.ptr = clk_get( \
			&pdev->dev, s5p_vp_ctrl_private.clk.name);
