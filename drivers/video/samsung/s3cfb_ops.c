...
#ifdef CONFIG_VCM
static dma_addr_t s3cfb_phys_alloc(resource_size_t size, unsigned flag)
{
	struct s3cfb_global *fbdev;
	dma_addr_t phys_addr;

	if (flag == VCM_DEV_FIMD0)
		fbdev = get_fimd_global(0);
	else if (flag == VCM_DEV_FIMD1)
		fbdev = get_fimd_global(1);
	else
		return -ENOMEM;

#ifdef CONFIG_S5P_MEM_CMA
	phys_addr = (dma_addr_t)cma_alloc(fbdev->dev, "fimd", (size_t)size, 0);
#else
	phys_addr = s5p_get_media_memory_bank(S5P_MDEV_FIMD, 0);
#endif
	return phys_addr;
}

static void s3cfb_phys_free(struct vcm_phys *phys)
{
	BUG_ON(phys->count != 1);
#ifdef CONFIG_S5P_MEM_CMA
...
const static struct s5p_vcm_driver s3cfb_vcm_driver = {
	.tlb_invalidator = NULL,
	.pgd_base_specifier = NULL,
	.phys_alloc = s3cfb_phys_alloc,
	.phys_free = s3cfb_phys_free,
};
#endif
		...
		struct fb_fix_screeninfo *fix, int arg)
{
	...
	if (UMP_HANDLE_DD_INVALID == ump_wrapped_buffer) {
		...
		if (ump_dd_vcm_attribute_set(ump_wrapped_buffer, arg))
			return -ENOMEM;
#endif
	}
	return 0;
					...
					struct fb_info *fb, int fimd_id)
{
	...
	s5p_vcm = vcm_create_unified((SZ_64M), id, &s3cfb_vcm_driver);
	if (IS_ERR(s5p_vcm))
		return PTR_ERR(s5p_vcm);
	if (vcm_activate(s5p_vcm))
		dev_info(fbdev->dev, "[fb%d] : VCM activated", win->id);
	win->dev_vcm = s5p_vcm;
	win->dev_vcm_res = vcm_make_binding(win->dev_vcm, fix->smem_len, id, 0);

	if (IS_ERR(win->dev_vcm_res))
		return -ENOMEM;
#ifdef CONFIG_VCM_VMM
	win->kernel_vcm = get_vcm_vmm();
	if (win->kernel_vcm == NULL)
		return -ENOMEM;
#endif
	win->kernel_vcm_res =
		vcm_map(win->kernel_vcm, win->dev_vcm_res->phys, 0);
	if (IS_ERR(win->kernel_vcm_res))
		return -ENOMEM;

	fix->smem_start = win->dev_vcm_res->phys->parts->start;
	fb->screen_base = (void *)win->kernel_vcm_res->start;
#else
#ifdef CONFIG_S5P_MEM_CMA
...
#if MALI_USE_UNIFIED_MEMORY_PROVIDER
#ifdef CONFIG_UMP_VCM_ALLOC
	ump_vcm.vcm = win->dev_vcm;
	ump_vcm.vcm_res = win->dev_vcm_res;
	ump_vcm.dev_id = id;
...
#ifdef CONFIG_VCM
		vcm_destroy_binding(win->dev_vcm_res);
		vcm_unmap(win->kernel_vcm_res);
#else
#ifdef CONFIG_S5P_MEM_CMA
...
#if MALI_USE_UNIFIED_MEMORY_PROVIDER
	case S3CFB_GET_FB_UMP_SECURE_ID:
		{
